...
<%
	...
	if (StringUtils.isNotBlank(code)) {
		openid = new OauthUtil().getOpenid(code);

		user = userDao.getByOpenid(openid);//UserUtils.getByOpenid(openid);//UserUtils.getByLoginName(principal.getLoginName());
		if (user == null) {
			user = new User();
			useropenid = "1";
		}
				...
				<form id="loginForm" action="${ctx}/login" method="post">
					...
					<div class="input-group">
						<div class="input-row">
							...
							<input list="pasta" type="text" name="username" id="username" placeholder="请填写登录账号" value="<%=StringUtils.isBlank(user.getLoginName())?"":user.getLoginName()%>">
							<datalist id="pasta">
						...
						<c:if test="<%=StringUtils.isNotBlank(useropenid)%>">
							<div class="input-row">
								...
								<input type="text" name="mobile" id="mobile" class="required" placeholder="请输入联系人手机">
							</div>
						</c:if>
					</div>
...
//						if(data.userMobile != ""){
//							alert(data.userMobile);
//						}
						var url = "#index_section?index";
						if(mobile != null && mobile != ''){
							url = "#index_section?mobile=" + mobile + "&index";
						}
